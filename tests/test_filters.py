...
def test_fir_lowpass_zerolag():
    f = esfilters.fir_lowpass_zerolag(fs=SAMPLE_FREQ, cutoff_freq=15, transition_width=3, numtaps=int(SAMPLE_FREQ/15*4))
    result = f(CHIRP_0_40)
    return result

def test_fir_highpass_zerolag():
    f = esfilters.fir_highpass_zerolag(fs=SAMPLE_FREQ, cutoff_freq=15, transition_width=3, numtaps=int(SAMPLE_FREQ/15*4))
    result = f(CHIRP_0_40)
    return result
