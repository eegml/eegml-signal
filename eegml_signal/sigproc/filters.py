...
class LowPassIdealizedFilter(object):
    ...
    def __init__(self, f0hz, fs=1.0, order=0, window='hamming'):
        ...
        self.arr_coef = signal.firwin(order,cutoff=self.ratiof, window=window)
    ...
    def filter(self, x):
        return signal.lfilter(self.arr_coef, 1, x) # axis=-1, zi=None

    def filtfilt(self,x):
        return signal.filtfilt(self.arr_coef, [1], x)
...
class GaussianLowPassFilter(LowPassIdealizedFilter):
    ...
    def __init__(self, f0hz, fs=1.0, order=0, window=None):
        ...
        order = 8*self.sigma
        self.order=order
        self.arr_coef = signal.gaussian(self.order, self.sigma)
...
# signal.iirdesign(wp, ws, gpass, gstop, analog=0, ftype='ellip', output='ba')
...
class PassBand(object):
    ...
    def __init__(self, fs=1.0, passfreqs=[1.0,10.0], stopfreqs=[],
        ...
        self.ba = signal.iirdesign(wp=wp, ws=ws, gstop=gstop, gpass=gpass, ftype=ftype)

    def lfilter(self, x):
        return signal.lfilter(self.ba[0],self.ba[1], x)

    def filtfilt(self, x):
        return signal.filtfilt(self.ba[0],self.ba[1], x)
...
if __name__=='__main__':
    ...
    fs = int(1.0/si)
    nfreq = fs/2
    t = arange(0.0,10.0,si) 
    chrp = signal.chirp(t, 1.0, 10.0, 40.0)
