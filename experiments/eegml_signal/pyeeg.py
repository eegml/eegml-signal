# copied over for easy availability into eegquant, but would prefer a pypi package -clm
...
def bin_power(X,Band,Fs):
	...
	Power_Ratio = old_div(Power,sum(Power))
...
def pfd(X, D=None):
	...
	for i in range(1,len(D)):
		if D[i]*D[i-1]<0:
			N_delta += 1
	n = len(X)
	return old_div(log10(n),(log10(n)+log10(old_div(n,n)+0.4*N_delta)))
...
def hfd(X, Kmax):
	...
	for k in range(1,Kmax):
		...
		for m in range(0,k):
			Lmk = 0
			for i in range(1,int(floor(old_div((N-m),k)))):
				Lmk += abs(X[m+i*k] - X[m+i*k-k])
			Lmk = Lmk*(N - 1)/floor(old_div((N - m), float(k))) / k
			Lk.append(Lmk)
		L.append(log(mean(Lk)))
		x.append([log(old_div(float(1), k)), 1])
...
def hjorth(X, D = None):
	...
	return sqrt(old_div(M2, TP)), sqrt(float(M4) * TP / M2 / M2)	# Hjorth Mobility and Complexity
...
#	import code;code.interact(local=locals())
	...
	Ap_En = old_div((Phi_m - Phi_mp), (N - M))
...
#			if max(abs(Emp[i] - Emp[j])) <= R: # v 0.01_b_r1
	...
	Samp_En = log(old_div(sum(Cm),sum(Cmp)))
